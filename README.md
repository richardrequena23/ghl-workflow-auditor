...
19 workflows audited. 2 findings: 2 high

Review Request After Close
--------------------------
~  [GHL010] Suppression tags are checked before the wait, never after  (Nudge with the link)
     why it matters: This workflow waits days before it asks. The check happens on the
     way in, so anyone who becomes unhappy DURING the wait is still asked. The longer
     the wait, the more likely that is.
     fix: Repeat the suppression branch immediately before each send. Evaluate at send
     time, not at enrollment time.
```
...
Most of these checks would produce nonsense one workflow at a time. Two examples of why
...
rule ships with a test that trips it **and** a test that must not trip it — 42 tests in
